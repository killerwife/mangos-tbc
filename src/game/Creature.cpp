...
SpellEntry const* Creature::ReachWithSpellAttack(Unit* pVictim)
{
    ...
    for (uint32 i = 0; i < CREATURE_MAX_SPELLS; ++i)
    {
        if (!m_spells[i])
            continue;
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(m_spells[i]);
        if (!spellInfo)
        {
...
{
    return SelectAttackingTarget(target, position, sSpellStore.LookupEntry<SpellEntry>(uiSpellEntry), selectFlags);
}
...
void Creature::ApplyGameEventSpells(GameEventCreatureData const* eventData, bool activated)
{
    ...
    if (remove_spell)
        if (SpellEntry const* spellEntry = sSpellStore.LookupEntry<SpellEntry>(remove_spell))
