        ...
        do
        {
            ...
            switch (temp.event_type)
            {
                ...
                case EVENT_T_SPELLHIT:
                    ...
                    {
                        SpellEntry const* pSpell = sSpellStore.LookupEntry<SpellEntry>(temp.spell_hit.spellId);
                        if (!pSpell)
                        {
                ...
                case EVENT_T_FRIENDLY_MISSING_BUFF:
                {
                    SpellEntry const* pSpell = sSpellStore.LookupEntry<SpellEntry>(temp.friendly_buff.spellId);
                    if (!pSpell)
                    {
                ...
                {
                    SpellEntry const* pSpell = sSpellStore.LookupEntry<SpellEntry>(temp.buffed.spellId);
                    if (!pSpell)
                    {
            ...
            for (uint32 j = 0; j < MAX_ACTIONS; ++j)
            {
                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_CAST:
                    {
                        const SpellEntry* spell = sSpellStore.LookupEntry<SpellEntry>(action.cast.spellId);
                        if (!spell)
                    ...
                    case ACTION_T_CAST_EVENT:
                        ...
                        if (!sSpellStore.LookupEntry<SpellEntry>(action.cast_event.spellId))
                    ...
                    case ACTION_T_CAST_EVENT_ALL:
                        ...
                        if (!sSpellStore.LookupEntry<SpellEntry>(action.cast_event_all.spellId))
                            sLog.outErrorEventAI("Event %u Action %u uses nonexistent SpellID %u.", i, j + 1, action.cast_event_all.spellId);
                        break;
                    case ACTION_T_REMOVEAURASFROMSPELL:
                        if (!sSpellStore.LookupEntry<SpellEntry>(action.remove_aura.spellId))
