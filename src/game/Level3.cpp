...
bool ChatHandler::HandleLearnAllMySpellsCommand(char* /*args*/)
{
    ...
    for (uint32 i = 0; i < sSkillLineAbilityStore.GetNumRows(); ++i)
    {
        ...
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(entry->spellId);
        if (!spellInfo)
            continue;
...
bool ChatHandler::HandleLearnCommand(char* args)
{
    ...
    if (!spell || !sSpellStore.LookupEntry<SpellEntry>(spell))
        return false;
...
bool ChatHandler::HandleDamageCommand(char* args)
{
    ...
    if (!spellid || !sSpellStore.LookupEntry<SpellEntry>(spellid))
        return false;
...
bool ChatHandler::HandleListTalentsCommand(char* /*args*/)
{
    ...
    for (PlayerSpellMap::const_iterator itr = uSpells.begin(); itr != uSpells.end(); ++itr)
    {
        ...
        SpellEntry const* spellEntry = sSpellStore.LookupEntry<SpellEntry>(itr->first);
        if (!spellEntry)
            continue;
