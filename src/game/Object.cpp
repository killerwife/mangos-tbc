...
bool WorldObject::IsFacingTargetsBack(const WorldObject* target, float arc) const
{
    if (!target)
        return false;
...
bool WorldObject::IsFacingTargetsFront(const WorldObject* target, float arc) const
{
    if (!target)
        return false;
