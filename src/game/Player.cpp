...
{
    SpellEntry const* affect_spell = sSpellStore.LookupEntry<SpellEntry>(spellId);
...
void Player::SendInitialSpells()
{
    ...
    {
        SpellEntry const* sEntry = sSpellStore.LookupEntry<SpellEntry>(itr->first);
        if (!sEntry)
            continue;
    ...
    if (!disabled_case) // skip new spell adding if spell already known (disabled spells case)
    {
        ...
        if (newspell.active && !newspell.disabled && sSpellMgr.IsRankedSpellNonStackableInSpellBook(spellInfo))
        {
            for (PlayerSpellMap::iterator itr2 = m_spells.begin(); itr2 != m_spells.end(); ++itr2)
            {
                ...
                SpellEntry const* i_spellInfo = sSpellStore.LookupEntry<SpellEntry>(itr2->first);
...
void Player::RemoveArenaSpellCooldowns()
{
    ...
    for (itr = m_spellCooldowns.begin(); itr != m_spellCooldowns.end(); itr = next)
    {
        next = itr;
        ++next;
        SpellEntry const* entry = sSpellStore.LookupEntry<SpellEntry>(itr->first);
...
TrainerSpellState Player::GetTrainerSpellState(TrainerSpell const* trainer_spell, uint32 reqLevel) const
{
    ...
    SpellEntry const* spell = sSpellStore.LookupEntry<SpellEntry>(trainer_spell->spell);
...
bool Player::IsActionButtonDataValid(uint8 button, uint32 action, uint8 type, Player* player)
{
    ...
    switch (type)
    {
        ...
        {
            SpellEntry const* spellProto = sSpellStore.LookupEntry<SpellEntry>(action);
            if (!spellProto)
            {
                if (player)
    ...
    if (isOutdoor)
    {
        ...
        for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
        {
            if (itr->second.state == PLAYERSPELL_REMOVED)
                continue;
            SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(itr->first);
...
void Player::ApplyItemEquipSpell(Item* item, bool apply, bool form_change)
{
    ...
    for (int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        SpellEntry const* spellproto = sSpellStore.LookupEntry<SpellEntry>(spellData.SpellId);
        if (!spellproto)
            continue;
...
void Player::CastItemCombatSpell(Unit* Target, WeaponAttackType attType)
{
    ...
    for (int i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(spellData.SpellId);
        if (!spellInfo)
        {
    ...
    for (int e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (int s = 0; s < 3; ++s)
        {
            ...
            SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(proc_spell_id);
            if (!spellInfo)
            {
...
void Player::CastItemUseSpell(Item* item, SpellCastTargets const& targets, uint8 cast_count)
{
    ...
    if (proto->Spells[0].SpellId == SPELL_ID_GENERIC_LEARN)
    {
        ...
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(learn_spell_id);
        if (!spellInfo)
        {
    ...
    if (!pItem2)
    {
        ...
        if (isAlive())
        {
            ...
            if (pProto && isInCombat() && (pProto->Class == ITEM_CLASS_WEAPON || pProto->InventoryType == INVTYPE_RELIC) && m_weaponChangeTimer == 0)
            {
                ...
                SpellEntry const* spellProto = sSpellStore.LookupEntry<SpellEntry>(cooldownSpell);
...
void Player::ProhibitSpellSchool(SpellSchoolMask idSchoolMask, uint32 unTimeMs)
{
    ...
    for (PlayerSpellMap::const_iterator itr = m_spells.begin(); itr != m_spells.end(); ++itr)
    {
        ...
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(unSpellId);
...
void Player::learnQuestRewardedSpells(Quest const* quest)
{
    ...
    if (sSpellMgr.GetSpellRank(learned_0) > 1 && !HasSpell(learned_0))
    {
        ...
        SpellEntry const* learnedInfo = sSpellStore.LookupEntry<SpellEntry>(learned_0);
        if (!learnedInfo)
            return;
        ...
        if (learnedInfo->Effect[EFFECT_INDEX_0] == SPELL_EFFECT_TRADE_SKILL && learnedInfo->Effect[EFFECT_INDEX_1] == 0)
        {
            ...
            for (PlayerSpellMap::const_iterator itr = m_spells.begin(); itr != m_spells.end(); ++itr)
            {
                ...
                SpellEntry const* itrInfo = sSpellStore.LookupEntry<SpellEntry>(itr->first);
                if (!itrInfo)
                    return;
...
void Player::learnSkillRewardedSpells(uint32 skill_id, uint32 skill_value)
{
    ...
    for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j)
    {
        ...
        if (sSpellStore.LookupEntry<SpellEntry>(pAbility->spellId))
        {
            // need unlearn spell
    ...
    void operator()(Unit* unit) const
    {
        if (unit->GetTypeId() == TYPEID_UNIT && ((Creature*)unit)->IsPet() && !((Pet*)unit)->isControlled())
            if (uint32 spell_id = unit->GetUInt32Value(UNIT_CREATED_BY_SPELL))
                if (SpellEntry const* spellEntry = sSpellStore.LookupEntry<SpellEntry>(spell_id))
