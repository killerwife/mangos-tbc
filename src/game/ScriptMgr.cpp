...
uint8 GetSpellStartDBScriptPriority(SpellEntry const* spellinfo, SpellEffectIndex effIdx)
{
    ...
    if (spellinfo->Effect[effIdx] == SPELL_EFFECT_TRIGGER_SPELL && !sSpellStore.LookupEntry<SpellEntry>(spellinfo->EffectTriggerSpell[effIdx]))
        return 5;

    // NonExisting trigger missile spells can also start DB-Spell-Scripts
    if (spellinfo->Effect[effIdx] == SPELL_EFFECT_TRIGGER_MISSILE && !sSpellStore.LookupEntry<SpellEntry>(spellinfo->EffectTriggerSpell[effIdx]))
        return 4;
...
void ScriptMgr::LoadScripts(ScriptMapMapName& scripts, const char* tablename)
{
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            {
                if (!sSpellStore.LookupEntry<SpellEntry>(tmp.removeAura.spellId))
                {
            ...
            {
                if (!sSpellStore.LookupEntry<SpellEntry>(tmp.castSpell.spellId))
                {
                    sLog.outErrorDb("Table `%s` using nonexistent spell (id: %u) in SCRIPT_COMMAND_REMOVE_AURA or SCRIPT_COMMAND_CAST_SPELL for script id %u",
                ...
                {
                    if (tmp.textId[i] && !sSpellStore.LookupEntry<SpellEntry>(uint32(tmp.textId[i])))
                    {
            ...
            case SCRIPT_COMMAND_SEND_TAXI_PATH:             // 30
            {
                ...
                {
                    uint32 taxiSpell = 0;
                    for (uint32 i = 1; i < sSpellStore.GetMaxEntry() && taxiSpell == 0; ++i)
                    {
                        if (SpellEntry const* spell = sSpellStore.LookupEntry<SpellEntry>(i))
...
void ScriptMgr::CollectPossibleEventIds(std::set<uint32>& eventIds)
{
    ...
    {
        SpellEntry const* spell = sSpellStore.LookupEntry<SpellEntry>(i);
        if (spell)
        {
            for (int j = 0; j < MAX_EFFECT_INDEX; ++j)
            {
