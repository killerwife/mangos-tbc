            ...
            switch (m_spellInfo->Id)
            {
                case 5938:                                  // Shiv
                {
                    ...
                    for (int s = 0; s < 3; ++s)
                    {
                        ...
                        SpellEntry const* combatEntry = sSpellStore.LookupEntry<SpellEntry>(pEnchant->spellid[s]);
        ...
        case SPELLFAMILY_PALADIN:
        {
            switch (m_spellInfo->SpellIconID)
            {
                ...
                case 561:                                   // Judgement of command
                {
                    ...
                    SpellEntry const* spell_proto = sSpellStore.LookupEntry<SpellEntry>(spell_id);
                    if (!spell_proto)
                        return;
...
void Spell::EffectTriggerSpellWithValue(SpellEffectIndex eff_idx)
{
    ...
    // normal case
    SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(triggered_spell_id);
...
void Spell::EffectLearnPetSpell(SpellEffectIndex eff_idx)
{
    ...
    SpellEntry const* learn_spellproto = sSpellStore.LookupEntry<SpellEntry>(m_spellInfo->EffectTriggerSpell[eff_idx]);
    if (!learn_spellproto)
        return;
...
void Spell::EffectStuck(SpellEffectIndex /*eff_idx*/)
{
    ...
    SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(8690);
    if (!spellInfo)
        return;
    ...
    for (int slot = 0;  slot < MAX_TOTEM_SLOT; ++slot)
    {
        if (Totem* totem = m_caster->GetTotem(TotemSlot(slot)))
        {
            if (damage)
            {
                ...
                if (SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(spell_id))
                {
...
void Spell::EffectStealBeneficialBuff(SpellEffectIndex eff_idx)
{
    ...
    if (!steal_list.empty())
    {
        ...
        if (!success_list.empty())
        {
            ...
            {
                SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(j->first);
