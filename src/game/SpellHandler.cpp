...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    if (pUser->isInCombat())
    {
        ...
        {
            if (SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(proto->Spells[i].SpellId))
            {
    ...
    if (!pItem->IsTargetValidForItemUse(targets.getUnitTarget()))
    {
        ...
        // send spell error
        if (SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(spellid))
...
void WorldSession::HandleSelfResOpcode(WorldPacket& /*recv_data*/)
{
    ...
    {
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(_player->GetUInt32Value(PLAYER_SELF_RES_SPELL));
        if (spellInfo)
