...
{
    SpellEntry const* spellInfo_1 = sSpellStore.LookupEntry<SpellEntry>(spellId_1);
    SpellEntry const* spellInfo_2 = sSpellStore.LookupEntry<SpellEntry>(spellId_2);
            ...
            switch (spellproto->EffectApplyAuraName[effIndex])
            {
                ...
                case SPELL_AURA_PERIODIC_TRIGGER_SPELL:
                    if (spellproto->Id != spellproto->EffectTriggerSpell[effIndex])
                    {
                        ...
                        SpellEntry const* spellTriggeredProto = sSpellStore.LookupEntry<SpellEntry>(spellTriggeredId);
...
{
    SpellEntry const* spellproto = sSpellStore.LookupEntry<SpellEntry>(spellId);
    if (!spellproto)
        return false;
...
struct SpellRankHelper
{
    ...
    {
        const SpellEntry* spell = sSpellStore.LookupEntry<SpellEntry>(spell_id);
        if (!spell)
        {
...
void SpellMgr::LoadSpellProcItemEnchant()
{
    ...
    do
    {
        ...
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(entry);
...
void SpellMgr::LoadSpellBonuses()
{
    ...
    do
    {
        ...
        SpellEntry const* spell = sSpellStore.LookupEntry<SpellEntry>(entry);
        if (!spell)
        {
...
SpellEntry const* SpellMgr::SelectAuraRankForLevel(SpellEntry const* spellInfo, uint32 level) const
{
    ...
    {
        SpellEntry const* nextSpellInfo = sSpellStore.LookupEntry<SpellEntry>(nextSpellId);
        if (!nextSpellInfo)
            break;
...
void SpellMgr::LoadSpellChains()
{
    ...
    for (unsigned int i = 0; i < sTalentStore.GetNumRows(); ++i)
    {
        ...
        for (int j = 0; j < MAX_TALENT_RANK; ++j)
        {
            ...
            if (!sSpellStore.LookupEntry<SpellEntry>(spell_id))
            {
    ...
    // load known data from spell abilities
    {
        ...
        for (SkillLineAbilityMap::const_iterator ab_itr = mSkillLineAbilityMap.begin(); ab_itr != mSkillLineAbilityMap.end(); ++ab_itr)
        {
            ...
            SpellEntry const* spell_entry = sSpellStore.LookupEntry<SpellEntry>(spell_id);
            ...
            // some forward spells not exist and can be ignored (some outdated data)
            SpellEntry const* forward_entry = sSpellStore.LookupEntry<SpellEntry>(forward_id);
            if (!forward_entry)
                continue;
    ...
    do
    {
        ...
        if (node.prev != 0 && !sSpellStore.LookupEntry<SpellEntry>(node.prev))
        {
        ...
        if (!sSpellStore.LookupEntry<SpellEntry>(node.first))
        {
        ...
        if (node.req != 0 && !sSpellStore.LookupEntry<SpellEntry>(node.req))
        {
...
void SpellMgr::LoadSpellLearnSkills()
{
    ...
    {
        bar.step();
        SpellEntry const* entry = sSpellStore.LookupEntry<SpellEntry>(spell);
...
void SpellMgr::LoadSpellLearnSpells()
{
    ...
    do
    {
        ...
        if (!sSpellStore.LookupEntry<SpellEntry>(node.spell))
        {
    ...
    for (uint32 spell = 0; spell < sSpellStore.GetMaxEntry(); ++spell)
    {
        ...
        for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
        {
            if (entry->Effect[i] == SPELL_EFFECT_LEARN_SPELL)
            {
                ...
                if (!sSpellStore.LookupEntry<SpellEntry>(dbc_node.spell))
                    continue;
...
void SpellMgr::LoadSpellScriptTarget()
{
    ...
    {
        SpellEntry const* spellProto = sSpellStore.LookupEntry<SpellEntry>(itr->spellId);
        if (!spellProto)
        {
    ...
    if (!sLog.HasLogFilter(LOG_FILTER_DB_STRICTED_CHECK))
    {
        ...
        {
            SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(i);
            if (!spellInfo)
                continue;
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    do
    {
        ...
        {
            SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(spell);
            if (!spellInfo)
            {
            ...
            SpellEntry const* spellInfo2 = sSpellStore.LookupEntry<SpellEntry>(aura);
            if (!spellInfo2)
            {
...
bool SpellMgr::IsSpellValid(SpellEntry const* spellInfo, Player* pl, bool msg)
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        switch (spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_LEARN_SPELL:
            {
                SpellEntry const* spellInfo2 = sSpellStore.LookupEntry<SpellEntry>(spellInfo->EffectTriggerSpell[i]);
...
void SpellMgr::LoadSpellAreas()
{
    ...
    do
    {
        ...
        if (!sSpellStore.LookupEntry<SpellEntry>(spell))
        {
        ...
        {
            SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(abs(spellArea.auraSpell));
            if (!spellInfo)
            {
...
void SpellMgr::CheckUsedSpells(char const* table)
{
    ...
    do
    {
        ...
        if (spell)
        {
            ...
            SpellEntry const* spellEntry = sSpellStore.LookupEntry<SpellEntry>(spell);
            if (!spellEntry)
            {
        ...
        else
        {
            ...
            {
                SpellEntry const* spellEntry = sSpellStore.LookupEntry<SpellEntry>(spellId);
                if (!spellEntry)
                    continue;
...
void SpellMgr::LoadSpellAffects()
{
    ...
    {
        SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(id);
        if (!spellInfo)
            continue;
