...
void Unit::JustKilledCreature(Creature* victim, Player* responsiblePlayer)
{
    ...
    SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(victim->GetUInt32Value(UNIT_CREATED_BY_SPELL));
...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss)
{
    ...
    SpellEntry const* spellProto = sSpellStore.LookupEntry<SpellEntry>(damageInfo->SpellID);
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    {
        SpellEntry const* createBySpell = sSpellStore.LookupEntry<SpellEntry>(gameObj->GetSpellId());
...
void Unit::RemoveGameObject(GameObject* gameObj, bool del)
{
    ...
    if (uint32 spellid = gameObj->GetSpellId())
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            SpellEntry const* createBySpell = sSpellStore.LookupEntry<SpellEntry>(spellid);
...
void Unit::Mount(uint32 mount, uint32 spellId)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else if (SpellEntry const* spellInfo = sSpellStore.LookupEntry<SpellEntry>(spellId))
        {
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    for (iter = tokens.begin(), index = ACTION_BAR_INDEX_START; index < ACTION_BAR_INDEX_END; ++iter, ++index)
    {
        ...
        // check correctness
        if (PetActionBar[index].IsActionBarForSpell() && !sSpellStore.LookupEntry<SpellEntry>(PetActionBar[index].GetAction()))
